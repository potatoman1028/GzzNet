...
gzz::GzzIOCP_EX::GzzIOCP_EX(const int maxIoThreadCount, const int maxSessionCount)
	: IocpHandle(NULL), CloseCount(0), MaxIoThreadCount(maxIoThreadCount), MaxSessionCount(maxSessionCount), Host(nullptr)
{
...
int gzz::GzzIOCP_EX::Open(unsigned short maxConnection, unsigned short port)
{
	...
	Host = new TcpListen(maxConnection, port, IocpHandle);

	for (int index = 0; index < maxConnection; index++)
	{
		TcpHost* _guest = new TcpHost(index, Host->GetSock(), IocpHandle, SessionItemPool[index]);
...
void gzz::GzzIOCP_EX::CloseAllSession()
{
