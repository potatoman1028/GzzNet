...
int gzz::SessionItem::AcceptProcess(int completionBytes)
{
	...
	TcpHost* _guest = static_cast<TcpHost*>(Socket);
...
{
	// 완료처리 함수를 스위칭 합니다. (Disconnect -> Accept)
	SwitchingConnected();
	...
	TcpHost* host = static_cast<TcpHost*>(Socket);
	int err = static_cast<int>(NET_ERR::SUCESS);
	...
	// 다시 AcceptEx를 겁니다.
	err = host->Accept(ListenSock, static_cast<LPOVERLAPPED>(this));
...
{
	// 완료처리 함수를 스위칭 합니다. (Disconnect -> Connect)
	SwitchingConnected();
