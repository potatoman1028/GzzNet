#include "TcpHost.h"
#include <iostream>
gzz::TcpHost::TcpHost(int id, SOCKET listen, HANDLE iocpHandle, LPOVERLAPPED overLapped)
	:TcpSocketEX(id)
{
	Open();
...
int gzz::TcpHost::Bind(SOCKET sock, HANDLE iocpHandle)
{
...
void gzz::TcpHost::Release()
{
	AsyncTcpRecv::Release();
	AsyncTcpSend::Release();
}

gzz::ISend* gzz::TcpHost::GetInterfaceSend()
{
	return static_cast<AsyncTcpSend*>(this);
}

gzz::IReceive* gzz::TcpHost::GetInterfaceRecv()
{
	return static_cast<AsyncTcpRecv*>(this);
}

int gzz::TcpHost::SetSocketOption(SOCKET listen)
{
	int _err = 0;
...
int gzz::TcpHost::Accept(SOCKET listen, LPOVERLAPPED overLapped)
{
