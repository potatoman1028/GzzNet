...
int gzz::AsyncTcpSend::SendMsg(const SOCKET& sock, const LPOVERLAPPED& overraped, PACKET& packet)
{
	...
	if (Write(packet))
	{
		// 남은데이터가 없다면 --> Completion Routin 중 처리가 완료되었다면 
		if (RemainSize == 0)
		{
			SwapBuffer();
...
int gzz::AsyncTcpSend::SendProcess(const SOCKET& sock, const int completionBytes, LPOVERLAPPED overlapped)
{
	std::lock_guard lock(WriteLock);
	// 다 보낸 경우.
